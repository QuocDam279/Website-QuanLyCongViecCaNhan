  ...
  return (
    <div className="flex flex-col h-auto p-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
        {[
          { label: 'Loại công việc', count: totalTypes, bg: 'bg-green-500' },
          { label: 'Số lượng công việc', count: totalJobs, bg: 'bg-red-500' },
          { label: 'Chưa thực hiện', count: todoCount, bg: 'bg-amber-500' },
          { label: 'Đang thực hiện', count: inProgressCount, bg: 'bg-blue-500' },
          { label: 'Đã hoàn thành', count: doneCount, bg: 'bg-pink-500' },
        ].map((item, index) => (
          <div
            key={index}
